...
##Use a bar plot to compare the average age of the contestants between the two shows
...
improved_data1
```
##I'm seeing how many contestants from the bachelor and bachelorette receieved a rose, to show their popularity

```{r}
improved_data1 %>%
  group_by(first_name, action) %>%
...
use the Eliminations data to make a new column that shows if the contestant received the first impression rose
```{r}
Eliminations %>%
...
make a shiny app all the details of seasons of the bachelor and bachelorette
```{r}
library(shiny)
```
